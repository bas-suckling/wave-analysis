...
function getSpeedArray(data) {
    ...
    for (let i = 0; i < data.length - 1 ; i++) {
        let speed = (geolib.getSpeed({
            "latitude": data[i].latitude,
            "longitude": data[i].longitude,
            "timestamp": (convertUnixTime(data[i].timestamp))
        }, {
            "latitude": data[i + 1].latitude,
            "longitude": data[i + 1].longitude,
            "timestamp": (convertUnixTime(data[i + 1].timestamp))
        }))      
...
function convertUnixTime(gpxTimeStamp) {
    ...
    let unixtTimeStamp = moment(timeStamp).unix().toString()
    console.log(unixtTimeStamp)
    return unixtTimeStamp
}
